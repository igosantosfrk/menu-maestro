...
const ProductFormDialog = ({ open, onOpenChange, tenantId, editing, categories, onSaved }: ProductFormDialogProps) => {
  const [form, setForm] = useState({
    ...
    track_stock: false, stock_quantity: '', stock_min_alert: '5',
  });
    ...
    if (open) {
      if (editing) {
        setForm({
          ...
          stock_min_alert: String(editing.stock_min_alert ?? 5),
        });
        // Load existing media
        loadMedia(editing.id);
      } else {
        setForm({ name: '', description: '', price: '', cost_price: '', category_id: '', is_available: true, is_featured: false, prep_time_min: '30', track_stock: false, stock_quantity: '', stock_min_alert: '5' });
        setMedia([]);
      }
    }
  }, [open, editing]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      stock_min_alert: form.track_stock ? (parseInt(form.stock_min_alert) || 5) : 5,
    };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {/* Estoque Section */}
          <div style={{ border: '1px solid #E8ECF4', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: form.track_stock ? 16 : 0 }}>
              <Switch checked={form.track_stock} onCheckedChange={v => setForm({ ...form, track_stock: v })} />
              <Label style={{ margin: 0, fontWeight: 600 }}>Controlar estoque</Label>
            </div>
            {form.track_stock && (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" min="0" value={form.stock_min_alert} onChange={e => setForm({ ...form, stock_min_alert: e.target.value })} placeholder="5" />
                </div>
              </div>
            )}
          </div>
