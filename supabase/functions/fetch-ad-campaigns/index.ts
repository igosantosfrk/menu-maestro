...
async function fetchMetaCampaigns(accessToken: string, accountId: string, dfShort: string, dtShort: string) {
  const cleanAccountId = accountId.startsWith('act_') ? accountId : `act_${accountId}`
  
  const url = `https://graph.facebook.com/v21.0/${cleanAccountId}/campaigns?fields=id,name,status,insights.time_range({"since":"${dfShort}","until":"${dtShort}"}){spend,impressions,clicks,actions}&filtering=[{"field":"effective_status","operator":"IN","value":["ACTIVE"]}]&access_token=${accessToken}`
  ...
  const accountUrl = `https://graph.facebook.com/v21.0/${cleanAccountId}/insights?fields=spend,impressions,clicks,actions&time_range={"since":"${dfShort}","until":"${dtShort}"}&access_token=${accessToken}`
...
async function fetchGoogleCampaigns(accessToken: string, customerId: string, dfShort: string, dtShort: string) {
  const cleanCustomerId = customerId.replace(/-/g, '')
  
  const query = `SELECT campaign.id, campaign.name, campaign.status, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions FROM campaign WHERE campaign.status = 'ENABLED' AND segments.date BETWEEN '${dfShort}' AND '${dtShort}'`
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      dateFrom = body.date_from || new Date(new Date().setHours(0,0,0,0)).toISOString()
      dateTo = body.date_to || new Date().toISOString()
    } catch {
      dateFrom = new Date(new Date().setHours(0,0,0,0)).toISOString()
      dateTo = new Date().toISOString()
    }
    const dfShort = dateFrom.split('T')[0]
    const dtShort = dateTo.split('T')[0]
    ...
    for (const account of (adAccounts || [])) {
      try {
        if (account.platform === 'meta') {
          const result = await fetchMetaCampaigns(account.access_token, account.account_id, dfShort, dtShort)
          allCampaigns.push(...result.campaigns)
          totalSpend += result.totalSpendToday
        } else if (account.platform === 'google') {
          const result = await fetchGoogleCampaigns(account.access_token, account.account_id, dfShort, dtShort)
          allCampaigns.push(...result.campaigns)
          totalSpend += result.totalSpendToday
        }
